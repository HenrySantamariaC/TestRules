...
import useSelectQuestions from '../hooks/useSelectQuestions.jsx'
    ...
    return (
        <div>
                ...
                <div className='flex justify-between items-center'>
                    <div className='flex gap-2'>
                        ...
                        <span className="whitespace-nowrap rounded-full bg-ui-colors-primary px-2.5 py-0.5 text-sm text-slate-50">
                            Licencia {test.clase} {test.categoria}
                        </span>
                    </div>
                    <BackRouteButton/>
                </div>
                <h2 className='text-2xl font-bold'>Evaluación {test.clase} {test.categoria}</h2>
                <span className='flex items-center gap-2 text-ui-colors-accent'>
                    Tiempo {test.duracion} min.
                    <TimerSolid />
                </span>
            </div>
            <div className='flex flex-col gap-2 bg-ui-colors-neutral p-4 mb-4 rounded-xl'>
                <h3 className='text-xl font-bold'>Descripción</h3>
                <p>El examen contiene {test.totalPreguntas} preguntas y tiene una duración de {test.duracion} minutos. Para su aprobación, el postulante deberá acertar por lo menos {test.minAprobar} respuestas de las {test.totalPreguntas} preguntas. </p>
            ...
            <Link
                ...
                to={replaceParam(PATHROUTES.QUESTION,':testId', test.id)}
            >
                Iniciar
            </Link>
        </div>
